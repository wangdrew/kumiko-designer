...
let colorFamily=null,customTarget='insert';
...
function persist(){try{localStorage.setItem(KEY,JSON.stringify(state));storageError=false;$('save-state').textContent='● Saved on this device';}catch{$('save-state').textContent='○ Save to disk to keep changes';storageError=true;}}
function change(fn){const before=JSON.stringify(state);fn();if(JSON.stringify(state)===before)return;resetConnectedCycle();closeContextMenu();past.push(before);if(past.length>100)past.shift();future=[];persist();render();}
function history(redo){const from=redo?future:past,to=redo?past:future;if(!from.length)return;resetConnectedCycle();closeContextMenu();to.push(JSON.stringify(state));state=JSON.parse(from.pop());colorFamily=null;selected.clear();persist();render();}
...
function apply(id,target){const ids=selected.size?[...selected]:target?[target]:[];if(!ids.length)return;change(()=>{for(const key of ids){if(id===0)delete state.inserts[key];else state.inserts[key]={patternId:id,colorId:state.paletteColorId};}});activePattern=id;renderPalette();}
...
$('custom-color-form').onsubmit=e=>{
 ...
 change(()=>{const c=rememberCustom(hex,name);if(customTarget==='insert'){state.paletteColorId=c.id;colorFamily='custom';}else state.config[customTarget]=c.hex;});
 $('custom-color-dialog').close();
};
$('filament-family').onchange=e=>{colorFamily=e.target.value;const first=colors().find(c=>c.family===colorFamily);if(first)change(()=>state.paletteColorId=first.id);renderPalette();};
...
$('board').addEventListener('dragleave',e=>{if(!$('board').contains(e.relatedTarget))clearHover();});$('board').addEventListener('drop',e=>{e.preventDefault();const id=e.target.closest('[data-cell]')?.dataset.cell;if(id&&dragPattern!==null)apply(dragPattern,id);clearHover();dragPattern=null;});
...
$('canvas').addEventListener('dblclick',e=>{
 if(e.button!==0||spaceHeld||e.target.closest('[data-canvas-control]'))return;
...
$('canvas').addEventListener('pointerdown',e=>{
 ...
 gesture={x:e.clientX,y:e.clientY,cell:cellAt(e.clientX,e.clientY),add:e.metaKey||e.ctrlKey,base:new Set(selected),moved:false,pointer:e.pointerId,mode:spaceHeld||e.button===1?'pan':'select',panStart:{...viewPan},scale:$('board').getScreenCTM().inverse().a};
...
$('canvas').addEventListener('pointerup',()=>{
 ...
 gesture=null;$('marquee').style.display='none';$('canvas').classList.remove('panning');if(g.mode==='select')showRotation(g.cell);
});
$('canvas').addEventListener('pointercancel',()=>{if(gesture?.mode==='select')selected=gesture.base;gesture=null;$('marquee').style.display='none';$('canvas').classList.remove('panning');updateSelection();});
document.addEventListener('keydown',e=>{
 if(e.target.closest('input,select,textarea,[contenteditable="true"]')||$('parts-dialog').open||$('custom-color-dialog').open||$('new-design-dialog').open||$('frame-color-popover').matches(':popover-open')||!$('insert-menu').hidden)return;
...
function renderParts(){
 ...
 const panelImage=document.createElement('img');panelImage.className='report-preview';panelImage.alt=`Panel preview: ${state.name||'Untitled panel'}`;
 panelImage.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(new XMLSerializer().serializeToString(previewSvg));report.append(panelImage);
 const rows=G.partsList(state),total=rows.reduce((n,r)=>n+r.quantity,0),board=G.board(state.config);
 ...
 const table=document.createElement('table');report.append(table);const head=table.createTHead().insertRow();
 for(const title of ['Design','Filament color','Hex','Full','Half','Total']){const th=document.createElement('th');th.scope='col';th.textContent=title;head.append(th);}
 ...
 const foot=table.createTFoot().insertRow(),label=foot.insertCell();label.colSpan=3;label.textContent='Total inserts';for(const key of ['full','half','quantity'])text('td',String(rows.reduce((n,r)=>n+r[key],0)),foot);
...
render();if(storageError)$('save-state').textContent='○ Save to disk to keep changes';
})();
